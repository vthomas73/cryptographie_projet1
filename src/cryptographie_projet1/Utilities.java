...
public class Utilities {
	...
	public ProgramInformations verifArgs(String[] args) throws Exception {
		...
		if (!args[0].equals("-enc") && !args[0].equals("-dec"))
			throw new Exception("Le premier argument doit etre : -enc|-dec");
		programInfos.encryptionMode = args[0];
		if (args.length < 2 || !args[1].equals("-key"))
			throw new Exception("Le deuxieme argument doit etre : -key");
		if (args.length < 3)
			throw new Exception("Le troisieme argument doit etre la cle K...K");
		verifyHexa(args[2]);
		programInfos.setKey(args[2]);
		if (args.length < 4 || !args[3].equals("-in"))
			throw new Exception("Le quatrième argument doit etre : -in");
		if (args.length < 5)
			throw new Exception("Le cinquieme argument doit etre le chemin d'entree <input file>");
		...
		if (args.length < 6 + nbFileList || !args[5 + nbFileList].equals("-out"))
			throw new Exception("Le sixieme argument doit etre : -out");
		if (args.length < 7 + nbFileList)
			throw new Exception("Le septieme argument doit etre le chemin de sortie <output file>");
	...
	private void verifyHexa(String key) throws Exception {
		...
		if (key.length() != 32 && key.length() != 48 && key.length() != 64)
			throw new Exception("La cle n'est pas codee sur 128/192 ou 254 bits !");
		if (!isHex)
			throw new Exception("La cle n'est pas au format hexadecimal");
	}

	private void verifyFileExists(String location) throws Exception {
		File f = new File(location);
		if (!f.exists()) {
			throw new Exception("Le chemin d'entree -in " + location + " n'existe pas");
		}
		if (f.isDirectory())
			throw new Exception("Le chemin d'entree -in " + location + " n'est un dossier");
	}
		...
		if (f.exists()) {
			...
			if (!inputString.toLowerCase().equals("yes"))
				throw new Exception("Le chemin de sortie -out " + location
						+ " existe deja et vous n'avez pas autorise le programme e ecraser le fichier");
		}
	}
